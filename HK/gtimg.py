    ...
    for stock_detail in stock_details:
        ...
        if stock[0][:4] =='v_hk' :
            ...
            stock_dict[fcode] = {
                ...
                'bid_volume': int(stock[7]) * 100,   #外盘
                'ask_volume': float(stock[8]) * 100, #内盘
                'bid1': float(stock[9]),
                'bid1_volume': int(stock[10]) * 100,
                'bid2': float(stock[11]),
                'bid2_volume': int(stock[12]) * 100,
                ...
                'Amount': float(stock[37]) * 10000,  # 换成英文
                'turnover': float(stock[38]) if stock[38] != '' else None,
                'PE': float(stock[39]) if stock[39] != '' else None,
                #'unknown': stock[40],
...
print(dat)
dat =  GetLatestQuoteStock( 'sh601857,sh600000' )
print(dat)